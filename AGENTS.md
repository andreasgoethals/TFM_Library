...
consumers deciding when to update.
...
3. **Papers are added ONLY when the owner explicitly says so** (or adds
   ...
   2. Extract text:
      `python scripts/extract_paper_text.py papers/<year>/<file>.pdf`
...
5. **Refreshing a dump:** run `python scripts/refresh_repositories.py`,
   then spot-check that symbol names cited in `SUMMARIES.md` /
   `REPOSITORIES.md` still exist; note the refresh in `CHANGELOG.md`.
...
9. **Never hand-edit `papers/text/**`** — those files are generated by
   `scripts/extract_paper_text.py`, so manual fixes are lost on the next
   extraction. Fix the script instead.
